...
#include <QMouseEvent>
#include <QMessageBox>
DoubleComputer::DoubleComputer(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::DoubleComputer)
{
    ui->setupUi(this);
    resize(1000, 840);
    a.clear();
...
DoubleComputer::~DoubleComputer()
{
    delete ui;
}
void DoubleComputer::paintEvent(QPaintEvent *e)
{
...
void DoubleComputer::mousePressEvent(QMouseEvent *e)
{
    int x, y;
...
int DoubleComputer::isWin(int x, int y)
{
    return f1(x, y) || f2(x, y) || f3(x, y) || f4(x, y);
}

int DoubleComputer::f1(int x, int y)
{
    int i;
    for(i = 0; i < 6; i++)
    {
...
int DoubleComputer::f2(int x, int y)
{
    int i;
    for(i = 0; i < 6; i++)
    {
...
int DoubleComputer::f3(int x, int y)
{
    int i;
    for(i = 0; i < 6; i++)
    {
...
int DoubleComputer::f4(int x, int y)
{
    int i;
    for(i = 0; i < 6; i++)
    {
...
void DoubleComputer::actionByAI()
{
    // 计算评分
    calculateScore();
...
// 最关键的计算评分函数
void DoubleComputer::calculateScore()
{
    // 统计玩家或者电脑连成的子
                        ...
                        if (!(y == 0 && x == 0))
                        {
                            ...
                            else if (botNum == 4)
                            {
                                if (emptyNum == 1)                // 死四
                                    ...
                                    scoreMapVec[row][col] += 1990; // 活四
                            }
                            else if (botNum >= 5)
                                scoreMapVec[row][col] += 300000;   // 活五，应该具有最高优先级
...
int DoubleComputer::isJinShou(int x, int y)
{
    return f5(x, y) || f6(x, y) ;
}

int DoubleComputer::f5(int x, int y)
{
...
int DoubleComputer::f6(int x, int y)
{
    int i, flag1 = 0, flag2 = 0, flag3 = 0, flag4 = 0;
    for(i = 0; i < 5; i++)
    {
...
void DoubleComputer::on_pushButton_4_released()
{
    emit mySignal();
    a.clear();
    for (int i = 0; i < 20; i++)
...
void DoubleComputer::on_pushButton_3_released()
{
    if(player % 2 == 1)
...
void DoubleComputer::on_pushButton_2_released()
{
...
void DoubleComputer::on_pushButton_released()
{
    a.clear();
    for (int i = 0; i < 20; i++)
    {
